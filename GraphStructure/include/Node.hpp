...
public:
	 ...
	 */
	void addNeighbor(std::string edgeName, Node<T> *const&newNeighbor);

	void addChild(std::string edgeName, Node<T> *const&childNode);

	void addParent(std::string edgeName, Node<T> *const&parentNode);
	 ...
	 */
	void removeEdgeBetween(Node<T> *const&otherNode);
	 ...
	 *  FUNCTIONS
	 ***********************************************/
	bool isNeighbor(Node<T> *const&otherNode);
	Edge<T>* getConnectingEdge(Node<T> *const&otherNode);
...
private:
	...
	bool isChildOf(Node<T> *const&possibleParent);
	bool isParentOf(Node<T> *const&possibleChild);
...
inline void Node<T>::addNeighbor(std::string edgeName,
		Node<T> *const&newNeighbor)
{
	this->addChild(edgeName, newNeighbor);
}

template<class T>
inline void Node<T>::addChild(std::string edgeName, Node<T> *const&childNode)
{
	...
	else
	{
		std::unique_ptr<Edge<T>> tempEdge(
				new Edge<T>(edgeName, this, childNode));

		childNode->inEdges.push_back(tempEdge.get());
...
template<class T>
inline void Node<T>::addParent(std::string edgeName, Node<T> *const&parentNode)
{
	parentNode->addChild(edgeName, this);
}

template<class T>
inline void Node<T>::removeEdgeBetween(Node<T> *const&otherNode)
{
	if (this->isNeighbor(otherNode))
	{
		if (this->isChildOf(otherNode))
		{
			...
			otherNode->removeOutEdge(edgeToRemove);
		}
		else if (this->isParentOf(otherNode))
		{
			otherNode->removeEdgeBetween(this);
		}
		else
		{
			badBehavior(__LINE__, __func__,
	...
	else
	{
		badBehavior(__LINE__, __func__,
				"Tried to remove an edge between node <" + this->getName()
						+ "> and node <" + otherNode->getName() + ">");
	}
}

template<class T>
inline bool Node<T>::isNeighbor(Node<T> *const&otherNode)
{
	return this->isChildOf(otherNode) || this->isParentOf(otherNode);
}

template<class T>
bool Node<T>::isChildOf(Node<T> *const&possibleParent)
{
	for (Edge<T> *currInEdge : this->inEdges)
	{
		if (currInEdge->getSourceNode() == possibleParent)
		{
			return true;
		}
	}
...
template<class T>
inline bool Node<T>::isParentOf(Node<T> *const&possibleChild)
{
	for (std::unique_ptr<Edge<T>> const &currOutEdge : this->outEdges)
	{
		if (currOutEdge.get()->getTargetNode() == possibleChild)
		{
			return true;
		}
	}
		...
		Node<T> *const&otherNode)
{
	if (this->isNeighbor(otherNode))
	{
		if (this->isChildOf(otherNode))
		{
			for (Edge<T> *currInEdge : this->inEdges)
			{
				if (currInEdge->getSourceNode() == otherNode)
				{
					return currInEdge;
				}
			}
		...
		else if (this->isParentOf(otherNode))
		{
			for (std::unique_ptr<Edge<T>> const &currOutEdge : this->outEdges)
			{
				if (currOutEdge.get()->getTargetNode() == otherNode)
				{
